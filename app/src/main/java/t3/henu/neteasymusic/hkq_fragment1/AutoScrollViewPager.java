...
public class AutoScrollViewPager extends ViewPager {
    ...
    int count = hkq_fragment.imgs.length;  //等于BlankFragment类中的imgs数组的长度
    ...
    public void selectImage(int index) {
        ...
        for (int i = 0; i < count; i++) {
            ImageView iv = (ImageView) hkq_fragment.dotLayout.getChildAt(i);
            if (i == index) {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case MSG_AUTO_SCROLL:
                    ...
                    Log.d("索引索引索引索引索引索引索引索引", getCurrentItem() + "");
                    selectImage(getCurrentItem());
